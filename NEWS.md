# fish_passage_peace_2025_reporting 0.15.0 (2026-08-05)

* Build the provincial Fish Data Submission workbook programmatically. `scripts/03_permit_submission/fds_prep_for_submission.R` reads the four step CSVs produced by the prep scripts, applies the submission-only transforms, and writes them into a copy of the blank provincial template — replacing five manual copy-paste-special steps. The submitted artifact is `data/permit_submission/PG25-983916.xlsx` under scientific fish collection permit PG25-983916. Derived columns are left for the workbook to compute: the VLOOKUP pulls from Step 1, Step 2's soak time, and all five Step 4 averages.
* Adopt the current provincial template, `FDS_Template2026-03-11.xlsx`. The version previously carried was roughly three years stale and in the legacy `.xls` format that `openxlsx` cannot open; column sets are identical across all four step sheets, so the change is a drop-in. The script resolves the newest shipped template rather than a pinned filename.
* Resolve watershed codes and waterbody identifiers from bcfishpass at submission time rather than inheriting them from the field-form geopackage, whose watershed data had gone stale — two crossings had valid 45-digit codes that were blank in the file. Crossing 199663 has no 1:50,000 cross-reference at all and now carries a temporary watershed code (TWC) with the watershed code and waterbody identifier left blank, per the template's own instructions.
* Point the data attachment at the permit submission workbook. It previously linked to `data/habitat_confirmations.xls`, which the 2025 pipeline no longer produces — habitat now comes from `form_fiss_site` and the five habitat averages are computed in R. The raw fish data link now carries the year suffix that matches the file on disk.
