...
headwaters around Arctic Lake is shown in
...
if (!is.null(municipalities) && nrow(municipalities) > 0L) {
...
```{r flood-map-detail, fig.cap = "South-east corner of the Parsnip River Watershed Group at full resolution — the headwaters around Arctic Lake on the continental divide. Symbology as Figure \\@ref(fig:flood-map-wsg); named streams labelled in italic blue; municipalities outlined and labelled in red where present."}
e <- sf::st_bbox(aoi)
...
if (!is.null(municipalities_inset) && nrow(municipalities_inset) > 0L) {
