...
Floodplain extent was modelled for the Parsnip River Watershed Group using the [`flooded`](https://newgraphenvironment.github.io/flooded/) R package, which combines a digital elevation model, the bull trout accessible stream network, and a modelled bankfull flood surface to identify low-relief ground reached at high flow. Inputs were the [MRDEM-30](https://open.canada.ca/data/en/dataset/18752265-bda3-498c-a4ba-9dfe68cb98da) 30 m DEM, the bull trout accessible stream network (stream order ≥ 3) from [bcfishpass](https://github.com/smnorris/bcfishpass), and BC Freshwater Atlas lakes and wetlands attached to that network. We used the **functional floodplain** scenario (`ff04` in `flooded::fl_scenarios()`) — the recurrent-inundation footprint. Full methodology, parameter values, and citations are in [Appendix - Floodplain Delineation](#app-floodplain).
...
Samples were collected using two approaches informed by @hobbs_etal2025EnvironmentalDNA: in-stream filtration with a Haltech EMOS sampler and grab-and-go bottle collection. For both methods, all water collected at a site (typically 1–3L) was filtered onto a single filter, yielding one replicate per site. Mixed cellulose ester (MCE) filters with 1µm pore size were used most commonly, with 0.45µm and 1.5µm filters of other materials used occasionally. Samples were typically collected upstream and downstream of each road–stream crossing, though in some cases only downstream samples were obtained.
...
To optimize laboratory efficiency, we collaborated with UNBC to develop a sample run plan that grouped samples by compatible dye types, minimizing the number of qPCR runs required. An [assay mapping table](https://github.com/NewGraphEnvironment/dff-2022/blob/main/data/edna_assays.csv) cross-referencing species codes to assay identifiers and dye types was developed, along with [processing scripts](https://github.com/NewGraphEnvironment/dff-2022/blob/main/scripts/edna_unbc_plan.R) to generate sample groupings and a [run order configuration](https://github.com/NewGraphEnvironment/dff-2022/blob/main/data/edna_group_run_order.csv) specifying species order within each group. 
