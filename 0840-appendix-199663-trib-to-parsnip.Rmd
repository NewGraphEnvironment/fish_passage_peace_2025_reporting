...
COST ESTIMATEs, CORRECT BCFISHPASS HABITAT MODELLING, and memo map (needs to be updated) SHOULD WORK ONCE THIS IS FIXED https://github.com/NewGraphEnvironment/fish_passage_template_reporting/issues/142
...
```{r map-199663, fig.cap = my_caption, eval = T}
 my_caption <- "Map of Tributary To Parsnip River"
 
 knitr::include_graphics("fig/gis/map_trib_to_parsnip.jpeg")
...
```{r tab-edna-results-199663-prep, include=FALSE}
edna_results_199663 <- edna_bystargets_peace |>
  dplyr::filter(grepl("^199663", site_id), !control_blank_field, !control_blank_office) |>
...
Should restoration/maintenance activities proceed, replacement of the `r fpr_my_pscis_info(col_pull = road_name)` crossing (`r as.character(my_site)`) with a streambed simulation (`r fpr::fpr_my_pscis_info(col_pull = recommended_diameter_or_span_meters)`m span) is recommended. At the time of reporting in 2026, the cost of the work is estimated at \$`r format(fpr::fpr_my_cost_estimate(), big.mark = ',')`.
