...
```{r ld-db}
# Read the locally-snapshotted parquet (refreshed via
# scripts/fp_inputs_snapshot.R; provenance in
# data/fp_inputs_snapshot_manifest.txt). Lets the report build from a
# fresh git clone with no DB or upstream-clone access at runtime.
...
**The snowpack signal is about timing, not quantity.** Total annual snowfall is essentially unchanged (`r sprintf("%+.0f", snowfall_p)` %); annual SWE is down `r sprintf("%.0f", -swe_p)` %. But the seasonal redistribution is sharp — summer SWE collapsed `r sprintf("%.0f", -summer_swe_p)` %, spring snowmelt rose `r sprintf("%+.0f", spring_melt_p)` %, and the snowmelt midpoint shifted `r sprintf("%.0f", -doy50_d)` days earlier. The freshet is arriving weeks earlier and the high-elevation snowpack that historically lingered into summer no longer does.
...
In `r params$project_year`, baseline or follow up monitoring data was gathered through completion of an effectivness montoring form (sites where remediation has been completed) and/or through baseline fish sampling and aquistion of aerial imagery at the following sites (Table \@ref(tab:tab-mon-overview):

- In `r params$project_year`, effectiveness monitoring and eDNA sampling was conducted on Tributary to Table River (PSCIS crossing 125231) on Table FSR. The initial habitat confirmation for the site is documented in @irvine2020ParsnipRiver [here](https://www.newgraphenvironment.com/Parsnip_Fish_Passage/). Baseline monitoring from 2023-2025 can be reviewed in  [here](https://www.newgraphenvironment.com/fish_passage_peace_2023_reporting/tributary-to-the-table-river---125231---appendix.html).

-  In `r params$project_year`, effectiveness monitoring and eDNA sampling was conducted on Tributary to Missinka River (PSCIS crossing 125179) on Chuchinka-Missinka FSR. Background information regarding the site can be reviewed [here](https://www.newgraphenvironment.com/fish_passage_peace_2022_reporting/) with `r params$project_year` results added to this memo here [here](https://www.newgraphenvironment.com/fish_passage_peace_2024_reporting/trib-to-missinka.html). 
