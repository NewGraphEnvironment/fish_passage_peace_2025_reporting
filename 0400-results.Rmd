...
The statistical-modelling methodology — channel-width prediction, stream-temperature SSN modelling with Poisson Consulting [@hill_etal2024Spatialstream; @hill_etal2025Spatialstream], the broader `fresh` + `link` habitat-modelling framework, and the calibration-against-`bcfishobs` trajectory — is described in the Methods chapter under Statistical Support for Habitat Modelling. The outputs feeding intrinsic-habitat and connectivity prioritization are summarised below under Statistical Habitat Modelling Outputs.
...
Outputs from the statistical-modelling work described in the methods (Channel Width, Stream Temperature and GSDD, and the broader habitat-modelling framework) feed downstream into our intrinsic-habitat-potential and connectivity-prioritization layers — both for `bcfishpass` (channel-width estimates contribute to species-specific linear spawning/rearing habitat models across BC) and for the project-specific work we run on top via [`fresh`](https://github.com/NewGraphEnvironment/fresh) and [`link`](https://github.com/NewGraphEnvironment/link). Stream-temperature predictions can exclude habitat reaches likely to be thermally limiting from intrinsic models, and Growing-Season Degree Days [@hill_etal2025Spatialstream] provides a productivity-relevant metric that ties habitat suitability to age-0 salmonid growth potential. The hydrometric / water-quality station network underpinning these analyses — organized through [`water-temp-bc`](https://www.newgraphenvironment.com/water-temp-bc) — will be documented in a forthcoming appendix listing the stations available in this study area along with their period-of-record and variable coverage.

<br>

The first concrete application of this framework is an Arctic grayling intrinsic-habitat and connectivity model for the FWCP Peace region, in development at the time of this report. Once published, the model layer will be added to the project's collaborative GIS environment for review and iteration, and predictions will be calibrated against known fish-presence observations from [`bcfishobs`](https://github.com/smnorris/bcfishobs) to quantify model uncertainty and surface opportunities for fish-passage restoration. Subsequent versions of this report (tagged at the URL referenced in the executive summary) will incorporate model outputs, calibration narrative, and figures as the work matures.
...
- **Tributary to Kerry Lake (PSCIS 198692)** — pre-remediation baseline (fish sampling and eDNA) to inform future effectiveness monitoring once the crossing is upgraded. The 2024 habitat confirmation is documented in @irvine_schick2025RestoringFish. Detail in [Appendix - Tributary to Kerry Lake](#trib-to-kerry-2025).
